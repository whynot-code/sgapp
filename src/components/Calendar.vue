    ...
    computed: {
        ...
        monthCells() {
            let cells = []
               ...
               for(let i = 1; i <= 42; i++) {
                   i >= this.firstMonthDay && i <= this.currMonthDays+this.firstMonthDay-1 ? cells.push([i ,i - this.firstMonthDay+1]) : cells.push([i])
               }
            
            return cells
        },
        firstMonthDay() { return new Date(this.year, this.month, 1).getDay() },
        currMonthDays() { return new Date(this.year, this.month+1, 0).getDate() }
    }
}
</script>
